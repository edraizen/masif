...
from masif.default_config.masif_opts import masif_opts, update_config
from masif.masif_modules.MaSIF_ppi_search import MaSIF_ppi_search
from masif.masif_modules.train_ppi_search import compute_val_test_desc
...
def masif_ppi_search_comp_desc(*ppi_list, eval=False, nn_model="masif.nn_models.masif_ppi_search.sc05.all_feat"):
    update_config("ppi_search", nn_model)
    ...
    if eval:
        eval_list = ppi_list
        pp_list = [mydir for mydir in os.listdir(parent_in_dir)]
    else:
        eval_list = []
    ...
    #        continue
        ...
        if len(l) > 0 and chain2 != "":
            ...
            mesh1 = Simple_mesh()
            mesh1.load_mesh(filenply_fn1ame)
            v1 = mesh1.vertices
            ...
            mesh2 = Simple_mesh()
            mesh2.load_mesh(filenply_fn1ame)
            v2 = mesh2.vertices
...
if __name__ == "__main__":
    ...
    # Read a list of pdb_chain entries to evaluate.
        ...
        if listfilename.startswith("{masif_source}"):
            listfilename = listfilename.format(masif_source=os.dirname(os.dirname(__file__)))

        with open(listfilename) as listfile:
            ppi_list = [line.rstrip() for line in listfile]

    else:
        sys.exit(1)

    masif_ppi_search_comp_desc(*ppi_list, eval=eval, nn_model=custom_params_file)
